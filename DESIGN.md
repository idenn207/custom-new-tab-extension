...
**범위: 달력 위젯 제외.** 달력 모드는 이제 자기 시각 문서를 갖는다 — 판정 기준은 `DESIGN.calendar.md` 에 있고, 구체 토큰은 코드가 생긴 뒤 scan 모드가 채우므로 현재 값을 이 문서의 시스템으로 굳히지 않는다. 이 문서가 다루는 것은 배경 레이어, 시계, 검색창, 바로가기(고정 + 사이드바), 배경 이미지 사이드바, 설정 모달, 저장소 고지 배너, 그리고 이들이 공유하는 유리 언어다. 달력 모드의 목표 층위는 이 문서와 갈리고, 재료는 이 문서에서 이름으로 상속한다.

문서에 남는 달력 관련 사실은 구조적인 것 하나뿐이다. **메인 위젯은 시계와 달력 중 택일이며 공존하지 않는다.** 새 메인 위젯을 만들 때 이 제약이 유지된다.

## 1. Overview: 사진 위의 유리판 [시계]
