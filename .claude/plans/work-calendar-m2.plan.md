...
**UI8과 UI15는 이 플랜이 직접 지지 않는다 — 후반부(`work-calendar-m2b.plan.md`)의
몫이다.** 둘 다 "M2에 포함한다"·"M2에서 표현된다"라고 말하고, 이 플랜과 후반부가
합쳐서 그 M2이므로 지켜진다. 표에 남겨 둔 이유는 **범위에서 빠진 것이 아니라 이사
갔다는 사실**을 여기서 읽을 수 있어야 하기 때문이다.
...
- **Action**:
  ...
  **이 Task가 새로 만드는 것 — 하나다.**
  1. `test/positioning.smoke.js` `runCalendarV4EquivalenceCases(collector)` — **고정 입력 넷을 반드시 담는다. 값을 여기 적는다**(R7 test F1·F4 — 값이 Validate 에만 있으면 Action 은 자기 완결이 아니다). 날짜는 전부 `shiftDateKey(calendar.todayKey, ±n)` 으로 만든다:
     ...
     **각 고정 입력 줄에 주석 표식 `DD3-FIXTURE` 를 단다.** Validation 2b 가 그 표식을 세어 넷 미만이면 죽는다. 1번 한 줄이 DD3 의 실질 판정이다 — v3 데이터는 전부 `startDate === endDate`라 **폭 없는 항목만으로는 DD3이 정반대로 구현돼도 통과한다.**
...
- **Validate**: `runCalendarV4EquivalenceCases(collector)`를 새로 만든다. **이 함수가 DD3·DD4의 기계적 단언을 산출하는 자리이며, 이름이 없으면 그 단언은 존재하지 않는다**(R0 test F1·F2·F4).
  ...
  - **무엇을 비교하는가. 기준값은 렌더에서 뜨지 않는다 — 계산한다.** `loadApp()`은 시동 마이그레이션이 끝난 뒤에야 반환하므로(`test/positioning.smoke.js:145` 부근, `newtab.js:4115-4116`) "마이그레이션 이전 렌더"는 Task 2 이후 **도달 불가능한 상태**다. 그것을 렌더로 뜨려는 어떤 설계도 같은 벽에 부딪히고, 두 투영이 **똑같이 틀린 채로** 맞아떨어져 버그가 통과한다.
    ...
    그러므로 v3의 마감 규칙을 하네스에 **참조 구현으로 적어 둔다.** 그 규칙은 한 줄이다 — `endDate`와 `todayKey`의 비교뿐이다(`newtab.js:2629-2631`). 고정 입력마다 그것을 적용한 기대값을 만들고, v4를 태운 뒤의 실제 값과 맞댄다.
...
- **Action**: PRD의 Open Question 1("연속 범위를 파생값으로 강등하는 마이그레이션")을 DD1~DD4 근거와 함께 해소 표기하고, M2 행의 Plan 칸이 **두 플랜을 모두 가리키는지** 확인한다. **M2 행의 Status는 `in-progress` 그대로 둔다** — 이 플랜만으로는 PRD의 M2 결과문("불연속 배치와 겹침이 그대로 표현된다")이 참이 되지 않는다. `complete` 표기는 후반부의 Task 5가 한다. **하루 재현 테스트를 실제로 수행하고 결과를 적는다**(UI11) — 지난 한 주의 실제 업무를 사후에 입력하고, (a) 관문이 실제 업무 경로를 담는가, (b) 작업당 실제로 쓰인 **이름 있는** 관문 수는 몇인가(DD9의 반증 지표), (c) 옮기는 비용이 실제로 줄었는가를 기록한다. **1~2개에 머물면 그 사실을 그대로 적는다** — PRD가 그것을 관문 모델 과잉의 신호로 미리 선언했고, 숨기면 반증 지표를 만든 이유가 사라진다.
...
- [ ] `[사람]` **내보내기 봉투가 가져오기의 판단 근거가 된다** — 맨 배열 파일은 v3 레거시로 들어오고(관문 재구성), `{version:4}` 봉투 파일에서 `gates` 없는 행은 거절되며, `version` 없는 객체는 통째로 거절되는지 셋을 각각 단언한다 (Task 1, santa R7 B1)
...
- [ ] `[기계]` PRD Open Question 1 해소 표기, M2 행 Plan 칸이 두 플랜을 가리키고 Status는 아직 `in-progress`
...
**`[사람]` 항목은 서른셋 중 스물여섯이다.** 그 스물여섯은 체크한다고 해서 참이 되지 않는다 —
러너도 CI도 커밋 훅도 없으므로 이 목록의 **서른셋 중 스물여섯**은 약속이지 게이트가
...
`[기계]` 다섯 · `[기계+사람]` 둘 · `[사람]` 스물여섯 = 서른셋이며, 셋을 더할 때마다
이 문단의 수를 같이 고친다.)
