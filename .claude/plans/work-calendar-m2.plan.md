...
**이것으로 강제되지 않는 서른둘이 강제되지는 않는다.** 바뀐 것은 그 서른둘이 강제되지
...
**`[사람]` 항목은 서른아홉 중 서른둘이다.** 그 서른둘은 체크한다고 해서 참이 되지 않는다 —
러너도 CI도 커밋 훅도 없으므로 이 목록의 **서른아홉 중 서른둘**은 약속이지 게이트가
...
`[기계]` 다섯 · `[기계+사람]` 둘 · `[사람]` 서른둘 = 서른아홉이며, 셋을 더할 때마다
이 문단의 수를 같이 고친다.)
