...
- **Action**:
  ...
  **이 Task가 새로 만드는 것 — 둘이다.**
  ...
  세부는 아래에 잇는다. 프로젝트가 하나도 없을 때 한 번 뜨는 안내를 만든다(DD13). 프로젝트 이름 하나를 받아 만들고, 건너뛰면 무소속으로 계속 쓴다. 다시 뜨지 않게 하는 플래그를 저장하되 **프로젝트를 만들지 않고 건너뛴 경우에도** 다시 뜨지 않는다 — 무소속이 정상 상태이므로 반복 안내는 강제가 된다(UI8).
...
decl() { printf '(^|[^A-Za-z0-9_])(function[[:space:]]+|const[[:space:]]+|let[[:space:]]+|var[[:space:]]+|async[[:space:]]+function[[:space:]]+)?%s[[:space:]]*([(=:]|$)' "$1"; }
...
shasum -a 256 -c .claude/plans/work-calendar-m2.baseline.sha256
